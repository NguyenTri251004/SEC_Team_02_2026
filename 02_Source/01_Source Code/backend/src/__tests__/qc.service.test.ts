...
describe("getAllTests", () => {
  ...
  it("supports multiple filters combined", async () => {
    ...
    await qcService.getAllTests({ lot_id: "lot-001", test_type: "Identity", result_status: "Pass" });
  ...
  it("supports date range filters", async () => {
    ...
    await qcService.getAllTests({ from_date: "2026-01-01", to_date: "2026-12-31" });
...
describe("updateTestResult", () => {
  const updatedTest = { ...sampleTest, result_status: "Pass", test_result: "Conforms", verified_by: "supervisor" };
...
describe("getQCStats", () => {
  it("calculates pass_rate correctly from 30-day data", async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({ rows: [{ test_type: "Identity", count: "10", pass_count: "8" }] }); // by type
    ...
    expect(stats.tests_by_type[0]).toMatchObject({ test_type: "Identity", count: 10, pass_count: 8 });
  });
...
describe("approveLot", () => {
  ...
  it("approves successfully when all tests pass", async () => {
    ...
    const queries: string[] = mockClient.query.mock.calls.map((c: unknown[]) => c[0] as string);
  ...
  it("calls client.release() even when error occurs", async () => {
    ...
    await expect(qcService.approveLot("lot-001", "user-1")).rejects.toThrow("DB error");
...
describe("rejectLot", () => {
  ...
  it("returns error when lot is not found", async () => {
    ...
    const result = await qcService.rejectLot("no-lot", "user-1", "Failed tests");
  ...
  it("rejects a Quarantine lot successfully and includes reason in notes", async () => {
    ...
    const result = await qcService.rejectLot("lot-001", "user-qc", "Identity test failed");

    expect(result.success).toBe(true);
    expect(result.message).toContain("từ chối");
    // The UPDATE params[0] should include the reason
    const updateCall = mockClient.query.mock.calls.find(
      (c: unknown[]) => (c[0] as string).includes("status = 'Rejected'")
    ) as unknown[];
  ...
  it("calls client.release() on error", async () => {
    ...
    await expect(qcService.rejectLot("lot-001", "user-1", "reason")).rejects.toThrow("DB fail");
